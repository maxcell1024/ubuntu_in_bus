...
if __name__ == '__main__':
    ...
    front_rgb_video = cv2.VideoWriter(front_rgb_filename, fourcc, rgb_fps, (rgb_w, rgb_h))  # 動画の仕様（ファイル名、fourcc, FPS, サイズ）
    front_ther_video = cv2.VideoWriter(front_ther_filename, fourcc, ther_fps, (ther_w, ther_h))  # 動画の仕様（ファイル名、fourcc, FPS, サイズ）
    ...
    rear_rgb_video = cv2.VideoWriter(rear_rgb_filename, fourcc, rgb_fps, (rgb_w, rgb_h))  # 動画の仕様（ファイル名、fourcc, FPS, サイズ）
    rear_ther_video = cv2.VideoWriter(rear_ther_filename, fourcc, ther_fps, (ther_w, ther_h))  # 動画の仕様（ファイル名、fourcc, FPS, サイズ）
    ...
    while(True):
        ...
        if dt_end < datetime.datetime.now():
            ...
            front_rgb_filename = front_output_dir + "front_rgb_video" + strdate + ".mp4"
            front_ther_filename = front_output_dir + "front_ther_video" + strdate + ".mp4"
            front_rgb_video = cv2.VideoWriter(front_rgb_filename, fourcc, rgb_fps, (rgb_w, rgb_h)) 
            front_ther_video = cv2.VideoWriter(front_ther_filename, fourcc, ther_fps, (ther_w, ther_h))

            rear_rgb_filename = rear_output_dir + "rear_rgb_video" + strdate + ".mp4"
            rear_ther_filename = rear_output_dir + "rear_ther_video" + strdate + ".mp4"
            rear_rgb_video = cv2.VideoWriter(rear_rgb_filename, fourcc, rgb_fps, (rgb_w, rgb_h)) 
            rear_ther_video = cv2.VideoWriter(rear_ther_filename, fourcc, ther_fps, (ther_w, ther_h))
